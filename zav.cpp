...
private:
    ...
    bool step(int x, int y, int startX, int startY, int prev_X, int prev_Y, int dir, int count_W, int move) { //рекурсивна функція для пошуку наступного кроку
        for (int turn = -1; turn <= 1; turn++) {            //цикл для спроби піти: проти годинникової стрілки (-1), прямо (0) та за годиниковою стрілкою (1)
            ...
            if (!isValid(next_X, next_Y) && !(next_X == startX && next_Y == startY)) continue;
            ...
            if (next_X == startX && next_Y == startY)
                return newDir == 3;

            if (step(next_X, next_Y, startX, startY, x, y, newDir, count_W, move + 1))
                return true;
